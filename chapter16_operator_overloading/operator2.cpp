...
MinMax operator+(const MinMax &m1, const MinMax &m2)
{
    int min{m1.getMin() < m2.getMin() ? m1.getMin() : m2.getMin()};
    int max{m1.getMax() > m2.getMax() ? m1.getMax() : m2.getMax()};

    return MinMax{min, max};
}

MinMax operator+(const MinMax &m, const int value)
{
    int min{m.getMin() < value ? m.getMin() : value};
    int max{m.getMax() > value ? m.getMax() : value};

    return MinMax{min, max};
}

MinMax operator+(const int value, const MinMax &m)
{
